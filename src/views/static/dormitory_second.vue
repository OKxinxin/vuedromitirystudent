    ...
    <div class="content">
      ...
      <div class="center">
        ...
        <div v-if="selectedBuildingName && selectedBuildingDormitories.length > 0">
          ...
          <el-table :data="selectedBuildingDormitories" style="width: 100%" @row-click="handleRowClick">
            ...
            <!-- <el-table-column type="expand">
              <template #default="scope">
                <el-table :data="selectedDormitoryStudent" style="width: 520px">
                  <el-table-column prop="studentId" label="学生ID" width="120" />
                  <el-table-column prop="studentName" label="学生姓名" width="120" />
                  <el-table-column prop="classinfo" label="班级" width="80" />
                  <el-table-column fixed="right" label="操作" width="120">
                    <template #default="scope">
                      <el-button link type="primary" size="small" @click="editdormitoryStudent(scope.row)">迁移</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              </template>
...
import { getAllBuilding, getAllDormitories, addDormitory, editDormitory, deleteDormitory,getDormitoryById, getAllDormitoryStudents, getAllStudents } from '../../http/DormitoryManagement';
...
export default defineComponent({
  data() {
    return {
      ...
      // expandedRows: [], // 展开的行
      // DormitoryStudentID: [], // 存储选择宿舍对应的所有学生id数据
      // selectedDormitoryStudent: [], // 存储选择宿舍对应的所有学生数据
  ...
  mounted() {
    ...
    // this.getAllDormitoryStudentData();
  ...
  watch: {
    ...
    // selectedDormitoryStudent(newValue) {
    //   console.log('sxid', this.selectedDormitoryStudent);
    //   this.selectedDormitoryStudent = this.
    // },
  ...
  methods: {
    ...
    // getAllDormitoryStudentData(){
    //   getAllDormitoryStudents()
    //     .then(res => {
    //       res.data.dormitoryStudents.forEach(student => {
    //         if (student.dormitoryId === this.selectedBuildingDormitories.id) {
    //           this.DormitoryStudentID.push(student.studentId);
    //         }
    //       });
    //     })
    //     .catch(err => {
    ...
    //     .then(res => {
    //       if (res) {
    //         if (res.data.id == this.DormitoryStudentID) {
    //           this.selectedDormitoryStudent = res.data.dormitoryStudents; // 将宿舍学生数据存储到AllDormitoryStudent数组中
    //         }
    //       } else {
    //         console.log(res);
    //       }
  ...
  if (this.editingDormitory) {
    ...
    // editdormitoryStudent(dormitoryId) {
    //   editDormitoryStudent()
    //     .then(res => {
    //       console.log(res);
    //       if (res.data.dormitoryStudents.dormitoryId == this.dormitory.id) {
    //         this.student.dormitoryStudents.dormitoryId = { ...dormitoryId }; // 将点击的宿舍对象赋值给editingDormitory
    //       } else {
    //         console.log(res);
    //       }
    //     })
